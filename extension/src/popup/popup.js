...
async function handleJobOrbitLogin() {
    try {
        ...
        // Open the auth URL in a new tab — active: false keeps popup open
        ...
        chrome.tabs.create({ url: authUrl, active: true }, (tab) => {
            hideLoading();
            showLoading('Waiting for Job Orbit login...');
            console.log('[Popup] Auth tab opened:', tab.id);
            
            let resolved = false;
            
            const finish = async (success, errorMsg) => {
                if (resolved) return;
                resolved = true;
            ...
            // Poll chrome.storage every 2 seconds to detect when session is saved by service worker
            const pollInterval = setInterval(async () => {
                try {
                    // Check if session was saved by the service worker
                    const [syncResult, localResult] = await Promise.all([
                        new Promise(r => chrome.storage.sync.get(['jobOrbitSession'], r)),
                        new Promise(r => chrome.storage.local.get(['jobOrbitSession'], r))
                    ]);
                    
                    const session = syncResult.jobOrbitSession || localResult.jobOrbitSession;
                    
                    if (session && session.extensionToken && session.expiresAt > Date.now()) {
                        console.log('[Popup] ✅ Session detected in storage! Login successful.');
                        await finish(true);
                    }
                } catch (e) {
                    console.error('[Popup] Poll error:', e);
                }
            }, 1500);
            ...
            const timeoutId = setTimeout(() => {
                chrome.runtime.onMessage.removeListener(messageListener);
                if (!resolved) {
                    finish(false, 'Login timed out. Please try again.');
                }
            }, 3 * 60 * 1000);
            
            // Watch for tab being closed manually by user
            chrome.tabs.onRemoved.addListener(function onTabClosed(tabId) {
                if (tabId === tab.id) {
                    chrome.tabs.onRemoved.removeListener(onTabClosed);
                    // Give the service worker 3 more seconds to finish saving
                    setTimeout(async () => {
                        if (!resolved) {
                            const [syncResult, localResult] = await Promise.all([
                                new Promise(r => chrome.storage.sync.get(['jobOrbitSession'], r)),
                            ...
                            if (session && session.extensionToken && session.expiresAt > Date.now()) {
                                ...
                                await finish(false, 'Login cancelled or failed. Please try again.');
                            }
                        }
                    }, 3000);
                }
...
function displayJobTracking(applications) {
    ...
    if (!applications || applications.length === 0) {
        ...
        if (appliedStatus) appliedStatus.textContent = '0';
        return;
    }
    ...
    const appliedStatus = document.getElementById('appliedStatus');
    if (appliedStatus) appliedStatus.textContent = appliedCount;
