...
class AutofillOrchestrator {
    ...
    detectFormFields() {
        ...
        const inputElements = document.querySelectorAll('input, select, textarea, [role="textbox"], [contenteditable="true"], [role="radio"], [role="checkbox"], [role="option"]');
    ...
    getFieldLabel(element) {
        ...
        // Try aria-labelledby (Crucial for Google Forms)
        ...
        // Special case for Google Forms: check preceding siblings of parent elements for role="heading" or just plain text
        let curr = element;
        for (let i = 0; i < 4; i++) { // go up a few levels
            ...
            if (sibling && sibling.textContent.trim() && sibling.textContent.trim().length < 150) {
        ...
        for (const [fieldName, patterns] of Object.entries(fieldMapper)) {
            for (const pattern of patterns) {
                ...
                if (confidence > bestConfidence) {
                    ...
                    // Only return immediately for an absolute perfect match
                    if (confidence === 1.0) {
                        return bestMatch;
                    }
                }
    ...
    calculateMatchConfidence(label, pattern) {
        if (label === pattern) return 1.0;
        
        const labelWords = label.split(/[\s\-_/]+/).filter(w => w.length > 0);
        const patternWords = pattern.split(/[\s\-_/]+/).filter(w => w.length > 0);

        // Word-boundary based exact substring match (e.g. "first name" matches inside "please enter your first name")
        const paddedLabel = ` ${labelWords.join(' ')} `;
        const paddedPattern = ` ${patternWords.join(' ')} `;
        if (paddedLabel.includes(paddedPattern)) {
            // Give higher score if the lengths are closer (penalize "name" matching "first name")
            const lengthRatio = patternWords.length / labelWords.length;
            return 0.8 + (0.15 * lengthRatio);
        }
        ...
        try {
            ...
            if (element.tagName === 'SELECT') {
                ...
                if (role === 'radio' || role === 'checkbox' || role === 'option') {
                    console.log(`[Orchestrator] 📍 Filling ROLE=${role.toUpperCase()} field "${field.label}"`);
                    const labelText = field.label || element.getAttribute('aria-label') || element.innerText || '';
