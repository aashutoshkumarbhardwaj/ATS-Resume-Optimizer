    ...
    static async handleAuthChanged(message) {
        try {
            ...
            if (tokens) {
                ...
                // Construct the session using SessionManager
                // If this is running in service worker, we can require it
                if (typeof SessionManager !== 'undefined') {
                    await SessionManager.createSession({
                        extensionToken: tokens.access_token || tokens,
                        user: user_info
                    });
                } else {
                    // Fallback manual write if SessionManager is not available
                    const sessionData = {
                        extensionToken: tokens.access_token || tokens,
                        tokenType: 'Bearer',
                    ...
                    await chrome.storage.sync.set({
                        jobOrbitAuth: {
                            extensionToken: sessionData.extensionToken,
