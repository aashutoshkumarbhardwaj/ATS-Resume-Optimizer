...
class SessionManager {
    ...
    static async createSession(authData) {
        try {
            ...
            let userId = authData.user?.id || authData.userId;
            
            // If user info is missing but we have a token, decode it!
            const token = authData.extensionToken || authData.token;
            if (token && (!userObj || !userObj.email)) {
                try {
                    ...
                    console.log('[SessionManager] 🔓 Decoded user from token:', userObj);
                } catch (e) {
                    console.error('[SessionManager] Failed to decode user from token:', e);
                }
            }
